...
const page = async ({ params: { productId } }: Params) => {
  const { data } = await requist.get(`product/${productId}`);
  console.log(data);

  return (
    <div className="container productId">
      <div className="productIds-img">
        <Image
      ...
      <div className="productId-texts">
        <h1 className="productId-h1">{data.title}</h1>
        <p className="productId-p">{data.description}</p>
        <span className="productId-span">
          {data.price} <span className="productId-span__minus">-</span>
          <span className="productId-span__color"> so{"'"}m</span>
        </span>
      </div>
    </div>
  );
